...
public class HarustroyServiceImpl implements IHarustoryService {
	...
	public Map<String,Object> boardOne(BoardVO vo) {
		...
		String aeId = harustroyMapper.getWriterUserId(vo.getBoard_no());
		
		if(aeId.equals(id)) {
			int result2 = harustroyMapper.updateEcho(vo); 
		}
		return mvo;
	}
	...
	public String deleteHarustory(BoardVO vo) {
		...
		int result5 = harustroyMapper.deleteBell(vo);
	...
	public String replyInsert(ReplyVO vo) {
		...
		if(vo.getAe_id().equals(vo.getBoard_ae_id())) {
			vo.setCcg_r002("읽음");;
		}else {
			vo.setCcg_r002("안읽음");;			
		}
	...
	public String replyDelete(ReplyVO vo) {
		int result1 = harustroyMapper.replyBellDelete(vo);
		int result2 = harustroyMapper.replyDelete(vo);
		String status;
