...
public class ExcelUtils {
	...
	public static void setList(XSSFWorkbook workbook, int rownum, int cellnum, List<PoMngVO> poList) throws IOException {
		...
		for (int i = 0; i < poList.size(); i++) {
			...
			evaluator.evaluateInCell(cell5);
		...
		Cell cell6 = sheet.getRow(9).getCell(2);
		Cell cell7 = sheet.getRow(22).getCell(3);
		Cell cell8 = sheet.getRow(22).getCell(5);
		
		cell6.setCellFormula("SUM(F13:F22)");	// 전체 합계
		cell7.setCellFormula("SUMIF(G13:G22,\"승인\",D13:D23) + SUMIF(G13:G22,\"\",D13:D23)"); // 수량 합계
		cell8.setCellFormula("SUM(F13:F22)"); // 전체 합계
		
		evaluator.evaluateInCell(cell6);
		evaluator.evaluateInCell(cell7);
		evaluator.evaluateInCell(cell8);
...
//	}
	...
	// 엑셀 파일 이름 가져오기
	public static String getExcelPo(String filePath, String br_no, String ae_id) {
		String savePath = filePath + "\\" + ae_id; 
		File dir = new File(savePath); 
		File saveFile = new File(savePath);
		if(!saveFile.exists()) {
