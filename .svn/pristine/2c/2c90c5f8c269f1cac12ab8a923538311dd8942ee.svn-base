...
<div class="board_wrap">
    ...
    <div class="board_write_wrap">
   	 <form action="/myHaru/mydiary?${_csrf.parameterName }=${_csrf.token}" method="post" id="diaryForm">
           ...
           <div class="title">
               <input type="text" placeholder="제목을 입력해주세요..." name="diary_title" id="diary_title" value="${dvo.diary_title }" maxlength="40">
           </div>
           <div class="cont">
        ...
        <c:if test="${status ne 'u'}">
        	<input type="button" id="autoInputBtn" class="on" value="자동완성"/>
...
$(function (){
	...
	autoInputBtn.on("click", function(){
		title.val("아버지 해방일지를 읽고");
		str = "아버지의 해방일지를 읽으며 아버지가 떠올랐다. \n";
		str += "한없이 강해보이시던 우리 아버지도,\n";
		str += "책속의 아버지도, \n";
		str += "아버지란 이름 아래 가려진 사람이였다.\n";
		str += "한 사람이었고, 아버지였고, 형제였고, 이웃이었다...\n\n";
		str += "오늘 아버지께 전화한통 드려야 겠다.\n\n";
		str += "감사합니다 아버지\n";
		str += "나도 언젠가 아버지가 되겠지..\n\n";
		str += "나도 언젠가 가려지겠지..\n";
		cont.val(str);
